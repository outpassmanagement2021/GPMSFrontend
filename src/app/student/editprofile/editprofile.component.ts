...
export class EditprofileComponent implements OnInit {
  ...
  async onEdit(value) {
    try {
      ...
      formData.append("parentEmail", value.parentEmail);
      ...
      let message = (
        await this.studentservice
          .updateSingleStudent(this.presentStudent._id, formData)
          .toPromise()
      ).message;
      this.messageService.add({
        key: "toastElement",
      ...
      this.router.navigateByUrl("v1/student/dashboard/profile");
    } catch (error) {
      console.log(error);
      this.messageService.add({
        key: "toastElement",
